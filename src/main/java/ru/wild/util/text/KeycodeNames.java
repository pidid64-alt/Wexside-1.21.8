...
public enum KeycodeNames {
   ...
   private static final Map<Integer, KeycodeNames> context = new HashMap<>();
   private static final Map<String, KeycodeNames> config = new HashMap<>();
   ...
   public static String handle(int var0) {
      ...
      if (var0 <= -100) {
         return "MOUSE" + (Math.abs(var0 + 100) + 1);
      }

      KeycodeNames var1 = context.getOrDefault(var0, KEY_NONE);
      return var1.instance;
   }

   public static int handle(String var0) {
      KeycodeNames var1 = config.getOrDefault(var0.toLowerCase(), KEY_NONE);
      return var1.data;
   }

   public static KeycodeNames process(int var0) {
      return context.getOrDefault(var0, KEY_NONE);
   }

   public boolean compute(int var1) {
      return var1 == this.process();
   }
   ...
   KeycodeNames(String var3, int var4) {
      this.instance = var3;
      this.data = var4;
   }

   static {
      for (KeycodeNames var3 : values()) {
         context.put(var3.data, var3);
         config.put(var3.instance.toLowerCase(), var3);
      }
   }
}
